...
from icm20948 import ICM20948 #library accelerometer
import time
import numpy as np
import integration_algo #accelerometer integration algorithm
...
def traitement_avec_algo(tracker, data, nb_data, multi_data): #integration algorithm and averaging
    ...
    while i < 1:
        ...
        if ident == "del":
            ...
            if i != -1:
                ...
                append_new_line(filename2, ', '.join([str(dat) for dat in unprocessed_data]))
            i = 0
            not_pressed = GPIO.input(BUTTON_GPIO)
            print("Press the button when you're ready")
            while not_pressed:
